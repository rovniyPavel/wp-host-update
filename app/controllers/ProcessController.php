...
class ProcessController extends BaseController {
	...
	public function actionIndex() {
		...
		$replace_options = ReplaceHelper::getReplaceOptions();
		$tables          = $replace_options['tables'];
		$step            = $replace_options['step'];
		$current_table   = $tables[ $step ];

		$rows_cnt       = ReplaceHelper::getRowCounts( $current_table );
		$run_query_info = $this->runSelectQuery( $current_table, $replace_options );
		$metric_end     = microtime( true );

		return $this->responseJson( array(
			'table'   => $current_table,
			'found'   => $rows_cnt,
			'updated' => $run_query_info['updated_rows'],
			'in'      => round( $metric_end - $metric_start, 3 ),
		) );
	}

	public function runSelectQuery( $current_table, $replace_options ) {

		global $wpdb;

		$result = array();
		if ( 'simple' === $replace_options['replace_method'] ) {
			$this->prepareSkipRules();
		}
		$updated_rows = 0;
		$rows_cnt     = ReplaceHelper::getRowCounts( $current_table );
		$primary_keys = ReplaceHelper::getPrimaryKeys( $current_table );
		...
		$blogs_replace = ( ! empty( $replace_options['domain_replace']['old_domain'] )
		                   && ! empty( $replace_options['domain_replace']['new_domain'] ) )
			? $this->prepareBlogReplace( $replace_options['domain_replace'] ) : '';
		...
		while ( $select_offset < $rows_cnt ) {
			...
			foreach ( $db_rows as $row ) {
				...
				foreach ( $row as $key => $value ) {
					...
					if ( 'simple' == $replace_options['replace_method'] && $this->canSkipColumn(
							ReplaceHelper::getCleanTableName( $current_table ),
							$key,
							$value ) ) {
						continue;
					}

					if ( $current_table == $wpdb->blogs || $current_table == $wpdb->site && ( ! empty( $blogs_replace ) ) ) {
						$new_value = ReplaceHelper::replace( $value, $blogs_replace );
					} else {
						$new_value = ReplaceHelper::recursiveReplace( $value, $replace_options['to_replace'] );
					}
	 ...
	 * @return array
	 */
	public static function prepareBlogReplace( $input ) {
